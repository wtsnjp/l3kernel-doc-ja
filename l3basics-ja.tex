...
%  (other).
  関数\meta{function}を\meta{name}（コロンより前の部分）と\meta{sigunature}%
  （コロンより後ろの部分）に分割します．得られた情報は3つのパートに分けて
  入力ストリームに置かれます：順に\meta{name}，\meta{signature}，そしてコロン
  が見つかったかどうかを表す論理トークン（これは変数を関数と区別するため）です．
  ここで\meta{name}にはエスケープ文字は含まれず，また\meta{name}および
  \meta{signature}はカテゴリーコード12の文字で構成されます．
\end{function}
...
次の3つの関数は\TeX マクロをその構成要素に分解します．渡された\meta{token}が
マクロでない場合は分解は行われません．その場合，3つの関数は\cs{scan_stop:}%
を入力ストリームに残します．
...
%  code $12$ (with spaces having category code $10$). Thus for example
  \meta{token}がマクロの場合，適用されている\TeX プレフィックスを入力
...
%  not a macro then \cs{scan_stop:} is left in the input stream.
  では|\long|が入力ストリームに残ります．もし\meta{token}がマクロで
  ない場合は\cs{scan_stop:}が入力ストリームに置かれます．
  \begin{texnote}
...
%  code $10$). Thus for example
  \meta{token}がマクロの場合，\TeX 言語レベルの引数仕様を入力ストリームに
  カテゴリーコード$12$の文字（空白文字はカテゴリーコード$10$）の列として
  残します．したがって
  \begin{verbatim}
    \cs_set:Npn \next:nn #1#2 { x #1 y #2 }
    \cs_argument_spec:N \next:nn
...
%  code $12$ (with spaces having category code $10$). Thus for example
  \meta{token}がマクロの場合，その置換テキストを入力ストリームに
  カテゴリーコード$12$の文字（空白文字はカテゴリーコード$10$）の列として
  残します．したがって
  \begin{verbatim}
    \cs_set:Npn \next:nn #1#2 { x #1~y #2 }
...
%  not a macro then \cs{scan_stop:} is left in the input stream.
  では\verb*|x#1 y#2|が入力ストリームに残ります．もし\meta{token}がマクロ
  でない場合は\cs{scan_stop:}が入力ストリームに置かれます．
  \begin{texnote}
%    If the argument specification contains the string |->|, then the
%    function produces incorrect results.
    引数仕様が文字列|->|を含む場合，この関数は正しく動作しません．
